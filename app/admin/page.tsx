...
function FallenLeavesView() {
  ...
  useEffect(() => {
    const leavesCollectionRef = query(
      collection(db, "fallenLeaves"),
      orderBy("date", "desc")
    );
    const unsubscribe = () =>
      onSnapshot(leavesCollectionRef, (snapshot) => {
    ...
    <div className="bg-red-200 rounded">
      ...
      <div>
        <div className="grid grid-cols-1 m-10">
          ...
          <div className="text-right mt-3">最終更新: {lastUpdateTime}</div>
        </div>
      </div>
      <a ref={anchorRef} className="hidden"></a>
    </div>
...
function BiomeCSVExportView() {
  ...
  useEffect(() => {
    const biomeCollectionRef = query(
      collection(db, "biome"),
      orderBy("date", "desc")
    );
    const unsubscribe = () =>
      onSnapshot(biomeCollectionRef, (snapshot) => {
...
function CameraStatusView() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = () =>
      onSnapshot(placeCollectionRef, (snapshot) => {
                ...
                <div className="w-full lg:max-w-full">
                  <div className="bg-white rounded lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
                    ...
                    <p className="text-gray-700 text-base text-left">
                      {place.name}: {place.congestion}人
                    </p>
                    <div className="flex items-right">
                      <div className="flex text-sm">
                        <p className="text-gray-600">
                          最終更新: {place.updatedAt}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
                    <div className="w-full">
                      <div className="grid grid-cols-2">
                        <div className="text-gray-900 font-bold text-xl text-left">
                          {user.nickName}
                        </div>
                        <div
                          className={`text-right ${
                            user.university ? "text-red-600" : ""
                          }`}
                        >
                          {user.university ? "在学中" : ""}
                        </div>
                      ...
                      <div className="grid grid-cols-2">
                        <p className="mt-2 mb-0 text-gray-700 text-xl text-left">
                          ポイント: {user.reward} pt
                        </p>
                    ...
                    <p className="text-right text-green-700">
                      {tokenList.includes(user.uid) ? "通知可能" : ""}
                    </p>
                  </div>
                </div>
              </div>
...
function NotifyView() {
  ...
  const onChangeBodyText = (event: React.ChangeEvent<HTMLInputElement>): void =>
  ...
  return (
    <div className="bg-orange-200 rounded">
      ...
      <div className="grid row-3 p-5">
        <div className="mb-2">
          <label htmlFor="title" className="flex text-right">
            タイトル
          </label>
          <input
            id="title"
        ...
        <div className="mb-2">
          <label htmlFor="message" className="flex text-right">
            通知メッセージ
          </label>
          <input
            id="message"
...
function NotificationView() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = () =>
      onSnapshot(notificationInfoCollectionRef, (snapshot) => {
                  ...
                  <div className="bg-white rounded lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
                    ...
                    <div className="grid grid-cols-2">
                      <div className="text-center text-base">
                        通知ユーザ:{" "}
                        {notification.pushUser.length === 0
                          ? "全員"
                          : notification.pushUser.length}
                      </div>
                      <div className="text-center text-base">
